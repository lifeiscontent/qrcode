...
import { appendBits, assert, getBit } from './utils';
import { Ecc } from './QrcodeModel/Ecc';
...
export class QrcodeModel {
  ...
  public static encodeText(text: string, ecl: Ecc): QrcodeModel {
    const segs: Qrsegment[] = Qrsegment.makeSegments(text);
    return QrcodeModel.encodeSegments(segs, ecl);
  }
  ...
  public static encodeBinary(data: readonly byte[], ecl: Ecc): QrcodeModel {
    const seg: Qrsegment = Qrsegment.makeBytes(data);
    return QrcodeModel.encodeSegments([seg], ecl);
  }
  ...
  public static encodeSegments(
    ...
    if (
      !(
        QrcodeModel.MIN_VERSION <= minVersion &&
        minVersion <= maxVersion &&
        maxVersion <= QrcodeModel.MAX_VERSION
      ) ||
      mask < -1 ||
      mask > 7
    )
    ...
    for (version = minVersion; ; version++) {
      const dataCapacityBits: int = QrcodeModel.getNumDataCodewords(version, ecl) * 8; // Number of data bits available
    ...
    for (const newEcl of [Ecc.MEDIUM, Ecc.QUARTILE, Ecc.HIGH]) {
      // From low to high
      if (boostEcl && dataUsedBits <= QrcodeModel.getNumDataCodewords(version, newEcl) * 8)
        ecl = newEcl;
    }
    ...
    const dataCapacityBits: int = QrcodeModel.getNumDataCodewords(version, ecl) * 8;
  ...
  public constructor(
    ...
    if (version < QrcodeModel.MIN_VERSION || version > QrcodeModel.MAX_VERSION)
  ...
  private addEccAndInterleave(data: readonly byte[]): byte[] {
    ...
    if (data.length != QrcodeModel.getNumDataCodewords(ver, ecl))
      throw new RangeError('Invalid argument');

    // Calculate parameter numbers
    const numBlocks: int = QrcodeModel.NUM_ERROR_CORRECTION_BLOCKS[ecl.ordinal][ver];
    const blockEccLen: int = QrcodeModel.ECC_CODEWORDS_PER_BLOCK[ecl.ordinal][ver];
    const rawCodewords: int = Math.floor(QrcodeModel.getNumRawDataModules(ver) / 8);
    const numShortBlocks: int = numBlocks - (rawCodewords % numBlocks);
    const shortBlockLen: int = Math.floor(rawCodewords / numBlocks);

    // Split data into blocks and append ECC to each block
    const blocks: byte[][] = [];
    const rsDiv: byte[] = QrcodeModel.reedSolomonComputeDivisor(blockEccLen);
    for (let i = 0, k = 0; i < numBlocks; i++) {
      ...
      k += dat.length;
      const ecc: byte[] = QrcodeModel.reedSolomonComputeRemainder(dat, rsDiv);
  ...
  private drawCodewords(data: readonly byte[]): void {
    if (data.length != Math.floor(QrcodeModel.getNumRawDataModules(this.version) / 8))
      throw new RangeError('Invalid argument');
  ...
  private getPenaltyScore(): int {
    ...
    for (let y = 0; y < this.size; y++) {
      ...
      for (let x = 0; x < this.size; x++) {
        if (this.modules[y][x] == runColor) {
          runX++;
          if (runX == 5) result += QrcodeModel.PENALTY_N1;
          else if (runX > 5) result++;
        } else {
          this.finderPenaltyAddHistory(runX, runHistory);
          if (!runColor)
            result += this.finderPenaltyCountPatterns(runHistory) * QrcodeModel.PENALTY_N3;
          runColor = this.modules[y][x];
          runX = 1;
        }
      }
      result +=
        this.finderPenaltyTerminateAndCount(runColor, runX, runHistory) * QrcodeModel.PENALTY_N3;
    }
    ...
    for (let x = 0; x < this.size; x++) {
      ...
      for (let y = 0; y < this.size; y++) {
        if (this.modules[y][x] == runColor) {
          runY++;
          if (runY == 5) result += QrcodeModel.PENALTY_N1;
      ...
      result +=
        this.finderPenaltyTerminateAndCount(runColor, runY, runHistory) * QrcodeModel.PENALTY_N3;
    }
    ...
    assert(0 <= k && k <= 9);
    result += k * QrcodeModel.PENALTY_N4;
  ...
  private static getNumRawDataModules(ver: int): int {
    if (ver < QrcodeModel.MIN_VERSION || ver > QrcodeModel.MAX_VERSION)
  ...
  private static getNumDataCodewords(ver: int, ecl: Ecc): int {
    return (
      Math.floor(QrcodeModel.getNumRawDataModules(ver) / 8) -
      QrcodeModel.ECC_CODEWORDS_PER_BLOCK[ecl.ordinal][ver] *
        QrcodeModel.NUM_ERROR_CORRECTION_BLOCKS[ecl.ordinal][ver]
    );
  }
  ...
  private static reedSolomonComputeRemainder(
    ...
    for (const b of data) {
      ...
      result.push(0);
      divisor.forEach((coef, i) => (result[i] ^= QrcodeModel.reedSolomonMultiply(coef, factor)));
    }
    return result;
  }
