...
export class QrcodeModel {
   ...
   */
  public static encodeBinary(data: readonly number[], ecl: Ecc): QrcodeModel {
  ...
  public static encodeSegments(
    ...
    for (version = minVersion; ; version++) {
      const dataCapacityBits: number = QrcodeModel.getNumDataCodewords(version, ecl) * 8; // Number of data bits available
    ...
    const dataCapacityBits: number = QrcodeModel.getNumDataCodewords(version, ecl) * 8;
    ...
    bb.forEach((b: number, i: number) => (dataCodewords[i >>> 3] |= b << (7 - (i & 7))));
  ...
  private addEccAndInterleave(data: readonly number[]): number[] {
    ...
    const numBlocks: number = QrcodeModel.NUM_ERROR_CORRECTION_BLOCKS[ecl.ordinal][ver];
    const blockEccLen: number = QrcodeModel.ECC_CODEWORDS_PER_BLOCK[ecl.ordinal][ver];
    const rawCodewords: number = Math.floor(QrcodeModel.getNumRawDataModules(ver) / 8);
    ...
    const blocks: number[][] = [];
    const rsDiv: number[] = QrcodeModel.reedSolomonComputeDivisor(blockEccLen);
    for (let i = 0, k = 0; i < numBlocks; i++) {
      ...
      k += dat.length;
      const ecc: number[] = QrcodeModel.reedSolomonComputeRemainder(dat, rsDiv);
      if (i < numShortBlocks) {
        dat.push(0);
      }
